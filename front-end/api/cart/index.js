...
*/
  ...
  app.post("/cart", function (req, res, next) {
    ...
    app.post("/checkout", function (req, res, next) {
        console.log("Attempting to checkout the cart: " + JSON.stringify(req.body));

        var custId = helpers.getCustomerId(req, app.get("env"));
        console.log("Customer id" + custId);

        var options = {
            uri: endpoints.ordersUrl + "/checkout",
            method: 'POST',
            json: true,
            body: req.body
        };

        console.log("Posting Check out: " + JSON.stringify(req.body));
        console.log(options);
        request(options, function(error, response, body) {
            if (error !== null ) {
            ...
            if (response.statusCode == 200 &&
                body != null && body != "") {
                if (body.error) {
                    console.log("Error with check out: " + err);
                    res.status(500);
                    res.end();
                    return;
                }
